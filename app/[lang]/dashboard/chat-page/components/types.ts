// app/[lang]/dashboard/chat-widget/page.tsx
// PromoHubAI – Live Chat Appearance (Tidio-inspired)
// Scope: Appearance only (backend save/load via /chat-widget/appearance)
// Stack: Next.js 14 + React + TailwindCSS

"use client";

import { useEffect, useMemo, useState } from "react";
import { Image as ImageIcon, Volume2, VolumeX, Loader2, Check } from "lucide-react";
import { apiFetch } from "@/lib/api";

// ✅ backend routes you showed:
// GET    /chat-widget/appearance/get/:widgetId
// POST   /chat-widget/appearance/create
// PUT    /chat-widget/appearance/update/:widgetId

// ✅ single-widget mode (you can later wire real id from DB/router)
const WIDGET_ID = "default";

// -----------------------------
// Typesexport type ChatPageConfig = {
  widgetId: string;
  createdAt?: string;
...
type ApiDoc = {
  // ✅ new backend contract adds exists flag
  exists: boolean;
...
function normalizeAppearance(input: any): WidgetAppearance {
  ...
  return {
    ...
    showWhenOffline:
      typeof a.showWhenOffline === "boolean" ? a.showWhenOffline : DEFAULT_APPEARANCE.showWhenOffline,
    enableSound: typeof a.enableSound === "boolean" ? a.enableSound : DEFAULT_APPEARANCE.enableSound,
  };
}
...
export default function ChatWidgetAppearancePage() {
  ...
  const [starters, setStarters] = useState<ConversationStarter[]>(DEFAULT_STARTERS);
  ...
  const enabledStarters = useMemo(() => starters.filter((s) => s.enabled), [starters]);

  function updateAppearance<K extends keyof WidgetAppearance>(key: K, value: WidgetAppearance[K]) {
  ...
  // GET /chat-widget/appearance/get/:widgetId
  // backend returns 200 with { data: { exists: boolean, ... } }
  // -----------------------------
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        // ✅ FIX 1: use WIDGET_ID (not hard-coded 1234)
        const res = await apiFetch(`/chat-widget/appearance/get/${WIDGET_ID}`, {
          method: "GET",
        ...
        // ✅ FIX 2: new contract: exists flag
        if (json.data?.exists === false) {
          setHasRecord("no");
          setAppearance(normalizeAppearance({}));
          setStarters(DEFAULT_STARTERS); // keep defaults (not empty)
          return;
        }
  ...
  async function onSave() {
    try {
      ...
      const payload = { widgetId: WIDGET_ID, appearance, starters };
      ...
      const doUpdate = () =>
        apiFetch(`/chat-widget/appearance/update/${WIDGET_ID}`, {
          method: "PUT",
          headers: { Accept: "application/json" },
          body: JSON.stringify(payload),
        });
      ...
      if (hasRecord === "yes") {
        ...
        // ✅ FIX: 409 conflict is NOT an exception. It's a response status.
        if (res.status === 409) {
          res = await doUpdate();
        }
      }
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Configure how your website widget looks before users open chat.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={onSave}
            disabled={hydrating || save.state === "saving"}
              ...
              {tab === "home" ? (
                <div className="space-y-8">
                  <SubSection title="Brand">
                    ...
                    <label className="text-sm space-y-1 block">
                      <span className="text-gray-600">Brand logo (optional)</span>
                      <div className="flex items-center gap-3">
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => {
                            ...
                            if (!file) return;
                            readImageFile(file, (dataUrl) => updateAppearance("brandLogo", dataUrl));
                          }}
                        />
                        ...
                        {appearance.brandLogo ? (
                          <button
                            ...
                            onClick={() => updateAppearance("brandLogo", undefined)}
                            type="button"
                          >
                            Remove
                          </button>
                  ...
                  <SubSection title="Conversation starters">
                    <p className="text-sm text-gray-500">
                      Visitors can quickly start a conversation. Each starter maps to a chatbot intent.
                    </p>
                      ...
                      {starters.map((s, index) => (
                        ...
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const copy = [...starters];
                              copy[index] = { ...copy[index], intent: e.target.value };
                              setStarters(copy);
                            }}
                          ...
                          <button
                            onClick={() => setStarters(starters.filter((x) => x.id !== s.id))}
                ...
                <div className="space-y-8">
                  <SubSection title="Background">
                    ...
                    <label className="text-sm space-y-1 block">
                      <span className="text-gray-600">Background image (optional)</span>
                      <div className="flex items-center gap-3">
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => {
                            const file = e.target.files?.[0];
                            if (!file) return;
                            readImageFile(file, (dataUrl) => updateAppearance("backgroundImage", dataUrl));
                          }}
                        />
                        <ImageIcon size={16} className="text-gray-400" />
                        {appearance.backgroundImage ? (
                          <button
                            className="text-xs text-gray-500 hover:text-red-500"
                            onClick={() => updateAppearance("backgroundImage", undefined)}
                            type="button"
                          >
                            Remove
                          </button>
                  ...
                  <SubSection title="Behavior">
                    ...
                    <Toggle
                      ...
                      icon={appearance.enableSound ? <Volume2 size={14} /> : <VolumeX size={14} />}
                    />

                    <Select
                      label="Launcher position"
                      value={appearance.position}
                      onChange={(v) => updateAppearance("position", v as any)}
                      options={[
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            <div className="relative rounded-2xl bg-gray-50 p-4">
              <div className="mx-auto w-[320px]">
                <div className="rounded-2xl overflow-hidden bg-white shadow-sm">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div className="ml-auto text-white/80 text-lg leading-none">⋮</div>
                    </div>

                    <div className="mt-3 text-lg font-semibold">{appearance.title}</div>
                    <div className="text-sm text-white/90">{appearance.message}</div>
                  </div>

                  <div className="p-4" style={{ backgroundColor: appearance.backgroundColor }}>
                    <div className="space-y-2">
                      {enabledStarters.length > 0 ? (
                        ...
                        <div className="text-sm text-gray-500">No conversation starters enabled.</div>
                      )}
                    </div>
                    ...
                    <div className="mt-4 rounded-xl bg-white/90 px-4 py-3">
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">
                        {appearance.showWhenOffline ? appearance.onlineStatus : appearance.offlineStatus}
                      </div>
                    </div>
                ...
                <div className={`mt-4 flex ${appearance.position === "right" ? "justify-end" : "justify-start"}`}>
                  <button className="rounded-full px-4 py-2 text-sm bg-white shadow-sm" type="button">
                    Chat with us 👋
                  </button>
                  <div
        ...
        {items.map((it) => (
          <button
            ...
            className={`relative pb-3 text-sm ${
              value === it.value ? "text-purple-700" : "text-gray-500 hover:text-gray-700"
            }`}
          >
            {it.label}
            {value === it.value ? (
