...
type ListResp = { success: true; data: { items: EventType[] } } | { success: false; error?: string };
...
export default function EventTypesPage() {
  ...
  async function load() {
    try {
      setHydrating(true);
      const res = await apiFetch(API.list, { method: "GET", headers: { Accept: "application/json" } });
  ...
  async function remove(id: string) {
    try {
      ...
      const res = await apiFetch(API.remove(id), { method: "DELETE", headers: { Accept: "application/json" } });
      const json = (await res.json().catch(() => null)) as SimpleResp | null;
    ...
    >
      <Card title="Create event type">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-4 items-end">
          <TextInput label="Event name" value={name} onChange={setName} placeholder="invoice.paid" disabled={saving} />
          <TextInput label="Description" value={desc} onChange={setDesc} placeholder="Triggered when payment is completed" disabled={saving} />
          <button
            type="button"
            onClick={create}
            disabled={saving}
      ...
      <Card
        ...
        right={<SearchBox value={q} onChange={setQ} placeholder="Search event types…" />}
      >
        {hydrating ? (
