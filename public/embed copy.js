(() => {
  ...
  const VERSION = "1.0.2-chatpage"; // bump to reduce cache confusion

  // Prevent double-injection (same version)
  ...
  const DEBUG = String(readAttr("data-debug") || q("debug") || "").trim() === "1";
  const log = (...args) => DEBUG && console.log("[PromoHubAI embed]", ...args);
  const warn = (...args) => DEBUG && console.warn("[PromoHubAI embed]", ...args);
  ...
  const chatPageId = (readAttr("data-chat-page-id") || q("chatPageId") || "").trim();
  ...
  // NOTE: companyId is OPTIONAL for widget appearance in your current setup,
  // but REQUIRED if you want PUBLIC chat-page read without session cookie.
  // We'll only enforce it when chatPageId is provided.
  if (chatPageId && !companyId) {
    console.error("[PromoHubAI embed] Missing companyId (required for public chat-page fetch)");
    return;
  }
  ...
  // Chat-page raw data (optional)
  let chatPage = null;

  // -----------------------------
  // Widget Settings (KEEP EXACTLY AS YOUR WORKING CODE)
  // ✅ Endpoint: GET /chat-widget/appearance/get/:widgetId
  // ✅ embed passes companyId via querystring so backend can read KV without session
  // -----------------------------
  async function loadWidgetSettings() {
    ...
    log("widget GET", { url, widgetId, companyId });
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (!res.ok) {
          lastErr = new Error(`widget http ${res.status}`);
        ...
        if (!json || json.success !== true || !json.data) {
          lastErr = new Error("invalid widget json");
          await sleep(250 * attempt);
          continue;
        }

        if (json.data.exists === false) {
          log("widget exists=false, using defaults");
          return;
        }
    ...
    warn("widget load failed, using defaults:", lastErr);
  }

  // -----------------------------
  // Chat Page Settings (NEW)
  // ✅ Endpoint: GET /chat-page/get/:chatPageId
  // IMPORTANT: Your current handler requires session cookie.
  // To make PUBLIC embed work, backend must accept companyId query param (like appearanceGet does).
  // We'll call with ?companyId=... and gracefully ignore failures.
  // -----------------------------
  async function loadChatPageSettings() {
    if (!chatPageId) return;

    const url =
      api(`/chat-page/get/${encodeURIComponent(chatPageId)}`) +
      (companyId ? `?companyId=${encodeURIComponent(companyId)}` : "");

    log("chat-page GET", { url, chatPageId, companyId });
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (!res.ok) {
          lastErr = new Error(`chat-page http ${res.status}`);
          await sleep(250 * attempt);
          continue;
        }
        ...
        if (!json || json.success !== true) {
          lastErr = new Error("invalid chat-page json");
          await sleep(250 * attempt);
          continue;
        }

        // contract from your handler: { success, exists, data }
        if (!json.exists || !json.data) {
          log("chat-page exists=false");
          return;
        }

        chatPage = json.data;
        log("chat-page loaded", {
          keys: chatPage && typeof chatPage === "object" ? Object.keys(chatPage) : [],
        });
        return;
      } catch (e) {
        lastErr = e;
    ...
    // Optional. We don't break widget if chat-page fails.
    warn("chat-page load failed (optional):", lastErr);
  }

  // -----------------------------
  // Merge chat-page into widget appearance (SAFE)
  // This matches your dashboard UI groups:
  //  - header: { pageTitle, welcomeMessage }
  //  - branding: { companyName, companyLogoUrl }
  //  - theme: { accentColor, userBubbleColor, backgroundColor, backgroundImage, position }
  // We only override if values are present.
  // -----------------------------
  function mergeChatPageIntoAppearance() {
    if (!chatPage || typeof chatPage !== "object") return;

    const header = chatPage.header && typeof chatPage.header === "object" ? chatPage.header : null;
    const branding =
      chatPage.branding && typeof chatPage.branding === "object" ? chatPage.branding : null;
    const theme = chatPage.theme && typeof chatPage.theme === "object" ? chatPage.theme : null;

    // header
    const pageTitle =
      (header && (header.pageTitle || header.title)) ||
      chatPage.pageTitle ||
      chatPage.title ||
      null;

    const welcomeMessage =
      (header && (header.welcomeMessage || header.message)) ||
      chatPage.welcomeMessage ||
      chatPage.message ||
      null;

    if (pageTitle) appearance.title = String(pageTitle);
    if (welcomeMessage) appearance.message = String(welcomeMessage);

    // branding
    if (branding) {
      if (branding.companyName) appearance.brandName = String(branding.companyName);
      if (branding.brandName) appearance.brandName = String(branding.brandName);

      if (branding.companyLogoUrl) appearance.brandLogo = String(branding.companyLogoUrl);
      if (branding.logoUrl) appearance.brandLogo = String(branding.logoUrl);
    }

    // theme
    if (theme) {
      // your UI likely uses accentColor as primary
      const accent = theme.accentColor || theme.brandColor || null;
      if (accent) {
        appearance.brandColor = String(accent);
        // keep widget behavior: actionColor drives button + user bubble
        appearance.actionColor = String(theme.userBubbleColor || theme.actionColor || accent);
      } else if (theme.userBubbleColor) {
        appearance.actionColor = String(theme.userBubbleColor);
      }

      if (theme.backgroundColor) appearance.backgroundColor = String(theme.backgroundColor);
      if (theme.backgroundImage) appearance.backgroundImage = String(theme.backgroundImage);

      if (theme.position === "left" || theme.position === "right") {
        appearance.position = theme.position;
      }
    }

    // optional starters override from chatPage (if you store them there)
    if (Array.isArray(chatPage.starters)) {
      starters = chatPage.starters;
    }

    log("chat-page merged", {
  ...
  function buildUi() {
    ...
    enabled.slice(0, 5).forEach((s) => {
      const item = create(
        "div",
        "sbtn",
        `<span>${sanitizeText(s.text)}</span><span style="opacity:.6">›</span>`
      );
  ...
  // -----------------------------
  (async () => {
    // 1) Widget works today -> keep it first
    await loadWidgetSettings();

    // 2) Optional chat-page -> merge into widget appearance
    await loadChatPageSettings();
    mergeChatPageIntoAppearance();
