...
export async function uploadToR2(env: EnvCommon, objectKey: string, buffer: ArrayBuffer, contentType?: string) {
...
export function r2UrlForKey(objectKey: string) {
  // We use the Worker proxy route /r2/<key> so front-end can fetch at same origin.
  return `/r2/${objectKey}`;
}
